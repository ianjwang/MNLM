...
#' @param autocorr_range Maximum range (in raster units) of spatial autocorrelation (default = 10). Can be set individually for each layer.
#' @param mag_var Magnitude of variation across the raster (default = 5). Can be set individually for each layer.
#' @param nug Magnitude of variation in the scale of autocorr_range; smaller values generate more homogeneous rasters (default = 0.2). Can be set individually for each layer.
...
mnlm_gaussianfield <- function(nlayers = 2, r, ncol, nrow, resolution = 1, autocorr_range = 10, mag_var = 5, nug = 0.2, mean = 0.5, user_seed = NULL, rescale = TRUE){
  ...
  if(length(autocorr_range) == 1) autocorr_range <- rep(autocorr_range, nlayers)
  if(length(mag_var) == 1) mag_var <- rep(mag_var, nlayers)
  if(length(nug) == 1) nug <- rep(nug, nlayers)

  for(i in 1:nlayers){
    nlm <- terra::rast(NLMR::nlm_gaussianfield(ncol, nrow, resolution = resolution, autocorr_range = autocorr_range[i], mag_var = mag_var[i],
                                         nug = nug[i], mean = mean, user_seed = user_seed, rescale = rescale))
    if(i == 1) nlm.s <- nlm
