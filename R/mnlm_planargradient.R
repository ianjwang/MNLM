...
#' @export
mnlm_planargradient <- function(nlayers = 2, r, ncol, nrow, resolution = 1, rescale = TRUE){
  if(length(r) == 1) r <- rep(r, nlayers - 1)

  for(i in 1:nlayers){
    nlm <- terra::rast(NLMR::nlm_planargradient(ncol = ncol, nrow = nrow, resolution = resolution, rescale = rescale))
    if(i == 1) nlm.s <- nlm
