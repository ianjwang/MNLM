...
#' @param roughness Controls the level of spatial autocorrelation (not equal to the Hurst exponent; default = 0.5). Can be set individually for each layer.
#' @param rand_dev Initial standard deviation for the displacement step (default = 1) that sets the scale of the overall variance in the resulting landscape. Can be set individually for each layer.
#' @param torus Logical value indicating whether the algorithm should be simulated on a torus (default = FALSE). Can be set individually for each layer.
...
mnlm_mpd <- function(nlayers = 2, r, ncol, nrow, resolution = 1, roughness = 0.5, rand_dev = 1, torus = FALSE, user_seed = NULL, rescale = TRUE){
  ...
  if(length(roughness) == 1) roughness <- rep(roughness, nlayers)
  if(length(rand_dev) == 1) rand_dev <- rep(rand_dev, nlayers)
  if(length(torus) == 1) torus <- rep(torus, nlayers)

  for(i in 1:nlayers){
    nlm <- terra::rast(NLMR::nlm_mpd(ncol = ncol, nrow = nrow, resolution = resolution, rescale = rescale,
                                     roughness = roughness[i], rand_dev = rand_dev[i], torus = torus[i]))
    if(i == 1) nlm.s <- nlm
